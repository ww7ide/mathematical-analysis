...
公理 1-3 显然是对 ``距离函数'' 非常自然的要求. 举例来说, 公理 3 可以被看作这样一条规则的公理化表述: ``从 $x$ 到 $y$ 的 `直达路径' 比先从 $x$ 到 $z$ 再从 $z$ 到 $y$ 的路径更短''. 而对距离非负的要求, 由公理 1-3 便可得到.
...
\begin{example}\label{ex:metric}
    \begin{enumerate}
        ...
        $$
        是 $X$ 上的度量, 称为离散度量.
        \item 设 $(X_j, d_j),\ 1 \leq j \leq m$ 是度量空间, $X \coloneqq X_1 \times \dots \times X_m$. 则函数
        $$
        d(x,y) \coloneqq \max_{1 \leq j \leq m} d_j(x_j, y_j)\ ,\quad x \coloneqq (x_1, \dots, x_m) \in X\ ,\quad y \coloneqq (y_1, \dots, y_m) \in X
        $$
...
\begin{proof}
    1. 由命题 \ref{prop:abs-in-Q} 可以直接验证 $\abs{\,\cdot\,}$ 满足度量公理.
\end{proof}
...
\begin{proof}
    2. 设 $U_1, U_2 \in \nbr(a)$. 故存在 $r_1, r_2 > 0$ 使得 $\ball(a, r_1) \in U_1$  和 $\ball(a, r_2) \in U_2$. 令 $r = \min\set{r_1, r_2}$, 于是
    $$
...
\begin{example}
    \begin{enumerate}
        ...
        $$
        则 $\lim z_n = 1 + 2\ii$.
        \item 常序列 $(a,a,a,\dots)$ 收敛到 $a$.
        \item 实数列 $\bigl((-1)^n\bigr)$ 发散.
        \item 设 $X$ 是度量空间 $(X_j, d_j),\ 1 \leq j \leq m$ 的积空间. 则序列 $(x_n) = \bigl((x_n^1, \dots, x_n^m)\bigr)$ 收敛到点 $a \coloneqq (a^1, \dots, a^m) \in X$ 当且仅当对于每个 $j \in \set{1, \dots, m}$ 都有序列 $(x_n^j)$ 收敛到点 $a^j \in X_j$.
    \end{enumerate}
\end{example}
...
\begin{proof}
    ...
    于是
    \begin{align*}
        \abs{z_n - (1 + 2\ii)} &= \abs*{\frac{1}{n+1} - \ii \frac{4}{n+2}} \\
...
\begin{exercises}
    证明序列 $(z_n)_{n \geq 1}$ 其中 $z_n \coloneqq (1 - 1/n)(1 + \ii)$ 收敛到 $1 + \ii$.
\end{exercises}
...
\begin{exercises}
    设 $X \coloneqq \set{z \in \C}{\abs{z} \leq 3}$ 并配备自然度量. 描述 $\bar\ball_X(0,3)$ 和 $\bar\ball_X(2,4)$, 并证明 $\bar\ball_X(0,3) \subset \bar\ball_X(2,4)$.
\end{exercises}
...
\begin{exercises}
    ...
    \begin{enumerate}
        \item $d(x,y) \coloneqq \abs{(1/x) - (1/y)}$ 是 $X$ 上的度量.
        \item 自然度量和 $d$ 是等价的.
...
\begin{exercises}
    设 $(X_j, d_j),\ 1 \leq j \leq n$ 是度量空间, $X \coloneqq X_1 \times \dots \times X_n$ 且 $d$ 是 $X$ 上的积度量. 证明
    $$
    \delta(x,y) \coloneqq \sum_{j=1}^n d_j(x_j, y_j)\ ,\quad x \coloneqq (x_1, \dots, x_m) \in X\ ,\quad y \coloneqq (y_1, \dots, y_m) \in X
    $$
    是 $X$ 上的度量且与 $d$ 等价.
\end{exercises}
...
\begin{exercises}[SNCF-度量]
    ...
    \delta(z,w) \coloneqq
    \begin{cases}
        & \abs{z-w}\ ,         & \text{若存在}\ \lambda > 0\ \text{使得}\ z = \lambda w\ , \\
        & \abs{z} + \abs{w}\ , & \text{其余情况}\ .
    \end{cases}
    $$
...
\begin{exercises}
    设 $(x_n)$ 是 $\C$ 中的序列且对于任意 $n \in \N$ 有 $\Re x_n = 0$. 证明, 若 $(x_n)$ 收敛到 $x$, 则 $\Re x = 0$.
\end{exercises}