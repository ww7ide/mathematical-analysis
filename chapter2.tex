...
\begin{definition}[数列]
    $\K$ 中的序列称为数列. 进一步, 将 $\K$ 中的所有数列, 即 $\K^\N$ 记作 $s$ 或 $s(\K)$, 于是 $s$ 构成了 $\K$-向量空间 (见例 \ref{ex:vector-spaces}.5). 最后, $\R$ 中的数列称为实数列, $\C$ 中的数列称为复数列.
\end{definition}

\begin{remark}
    \begin{enumerate}
        \item 区分序列 $(x_n)$ 和它的像 $\set{x_n}{n \in \N}$ 是很重要的. 例如, 若对于任意 $n$ 有 $x_n = x$, 也就是说 $(x_n)$ 是一个常序列, 此时 $(x_n) = (x, x, x, \ldots)$, 而 $\set{x_n}{n \in \N}$ 是单点集 $\{x\}$.
        \item 设 $(x_n)$ 是 $X$ 中的序列, $E$ 是某个性质. 称 $E$ 对 $(x_n)$ 的几乎所有项都成立, 若存在 $m \in \N$ 使得当 $n \geq m$ 时命题 $E(x_n)$ 都成立. 也就是说, 除了至多有限多个 $x_n$ 以外, $E$ 对其余所有 $x_n$ 都成立. 当然, $E$ 也可以对若干个 (甚至全部) $n < m$ 的项成立. 若存在 $N \subseteq \N$ 满足 $|N| = \aleph_0$ 且 $E(x_n)$ 对所有 $n \in N$ 都成立, 则称 $E$ 对 $(x_n)$ 的无穷多项都成立. 例如, 实数列
        $$
...
\begin{axiom}[度量]\label{axiom:metric}
    设 $X$ 是集合, 其上定义了函数
    $$
...
\hyperref[axiom:metric]{$(\mathrm M_1)$}-\hyperref[axiom:metric]{$(\mathrm M_3)$} 显然是对 ``距离函数'' 非常自然的要求. 举例来说, \hyperref[axiom:metric]{$(\mathrm M_3)$} 可以被看作这样一条规则的公理化表述: ``从 $x$ 到 $y$ 的 `直达路径' 比先从 $x$ 到 $z$ 再从 $z$ 到 $y$ 的路径更短''.
...
\begin{example}\label{ex:metric}
    \begin{enumerate}
        ...
        \item $\C$ 的任意非空子集, 在从 $\C$ 的自然度量诱导出来的度量之下, 都是度量空间. 以这种方式在 $\R$ 上得到的度量, 正好就是在例 \ref{ex:metric}.1 中定义的那个自然度量.
        \item 设 $X$ 是非空集合. 则函数
        $$
...
\begin{example}\label{ex:ez-convergent-sequence}
    \begin{enumerate}
...
\begin{example}\label{ex:bounded-sets}
    \begin{enumerate}
...
\begin{proof}
    令 $\psi_1(k) \coloneqq k^m$, $\psi_2(k) \coloneqq m^k$. 由于 $\psi_i \colon \N^\ast \to \N^\ast,\ i = 1,2$ 是严格递增的, 因此 $(k^{-m})$ 和 $(m^{-k})$ 是 $(1/n)$ 的子数列. 于是由例 \ref{ex:ez-convergent-sequence}.1 和 命题 \ref{prop:convergent-sequence-to-subsequence} 可得.
\end{proof}
...
\begin{proof}
    ...
    $$
    由 \hyperref[prop:well-ordering-principle]{良序原理} 可知, 对于每个 $k \in \N^\ast$, $n_k$ 是良定义的. 于是 $(n_k)$ 是良定义且严格递增的. 任取 $\varepsilon > 0$, 由推论 \ref{cor:archimedean-property-cor} 可知, 存在 $K \in \N$ 使得 $1/K < \varepsilon$, 由 $n_k$ 的定义可知
    $$
...
\begin{definition}[零序列]
    称 $\K$ 中的数列 $(x_n)$ 是零序列, 若 $(x_n)$ 收敛到 $0$. 进一步, 将 $\K$ 中的所有零序列记作
    $$
...
\begin{remark}\label{rem:null-sequences}
    ...
    \begin{enumerate}
        \item $(x_n)$ 是零序列, 当且仅当其绝对值数列 $(|x_n|)$ 是 $\R$ 中的零序列.
        \item $(x_n)$ 收敛到 $a$, 当且仅当 ``平移后的数列'' $(x_n - a)$  是零序列.
        \item 若存在 $\R$ 中的零序列 $(r_n)$ 使得对几乎所有 $n \in \N$ 都有 $|x_n| \leq r_n$, 则 $(x_n)$ 是零序列.
    \end{enumerate}
\end{remark}
...
\begin{remark}\label{rem:c-is-vector-space}
    ...
    $$
    是线性的. 显然 $\ker \lim = c_0$, 于是, 由例 \ref{ex:vector-spaces}.3 可知, $c_0$ 是 $c$ 的子空间.
\end{remark}
...
\begin{proposition}\label{prop:multiplication-in-c}
    ...
    \begin{enumerate}
        \item 若 $(x_n)$ 是零序列且 $(y_n)$ 有界, 则 $(x_n y_n)$ 是零序列.
        \item 若 $\lim x_n = a$, $\lim y_n = b$, 则 $\lim x_n y_n = ab$.
    \end{enumerate}
\end{proposition}

\begin{proof}
    1. 由于 $(y_n)$ 有界, 故存在 $M > 0$ 使得对于任意 $n \in \N$ 都有 $|y_n| \leq M$. 任取 $\varepsilon > 0$. 由于 $(x_n)$ 是零序列, 故存在 $N \in \N$ 使得当 $n \geq N$ 时 $|x_n| < \varepsilon / M$. 于是
    $$
    |x_n y_n| = |x_n| |y_n| < \frac{\varepsilon}{M} M = \varepsilon\ ,\quad n \geq N\ .
    $$
    2. 由于 $(x_n)$ 收敛到 $a$, 故 $(x_n - a)$ 是零序列. 由命题 \ref{prop:convergent-sequence-is-bounded} 可知, $(y_n)$ 有界. 于是由 1 可得 $\bigl((x_n - a)y_n\bigr)$ 是零序列, 同理可得 $\bigl(a(y_n - b)\bigr)$ 是零序列. 由命题 \ref{prop:addition-scalar-multiplication-in-c} 可知
    $$
...
\begin{remark}\label{rem:c-is-algebra}
    \begin{enumerate}
        ...
        \item 由例 \ref{ex:algebras}.2 可知 $s$ 是 $\K$-代数. 于是, 结合注 \ref{rem:c-is-vector-space} 和命题 \ref{prop:multiplication-in-c}.2 可以得到: $c$ 是 $s$ 的子代数且函数
        $$
        \lim \colon c \to \K\ ,\quad (x_n) \mapsto \lim x_n
        $$
...
\begin{proof}
    1. 设 $x_n \coloneqq 1/n$, $y_n \coloneqq n^2$. 则 $(x_n)$ 是零序列, 但 $(x_n y_n)$ 发散.
\end{proof}
...
\begin{proof}
    ...
    $$
    由于 $(x_n)$ 收敛到 $a$, 因此 $(x_n - a)$ 是零序列, 进一步, $2|x_n - a| / |a|^2$ 是零序列. 于是由注 \ref{rem:null-sequences}.3 可得 $(1/x_n - 1/a)$ 是零序列, 故 $\lim 1/x_n = 1/a$.
\end{proof}
...
\begin{exercises}
    使用 $(1+1)^n$ 的二项式展开, 证明 $(n^3 / 2^n)$ 是零序列.
\end{exercises}
...
\begin{exercises}
    设 $\K^m\ (m \in \N^\ast)$ 是配备积度量 (见例 \ref{ex:metric}.5) 的度量空间,
    $$
    ...
    \begin{enumerate}
        ...
        $$
        是线性的.
        \item 设 $(\lambda_n) \in c(\K)$, $(x_n) \in c(\K^m)$ 且 $\lim \lambda_n = \alpha$, $\lim x_n = a$. 则 $\lim \lambda_n x_n = \alpha a \in \K^m$ (提示: 例 \ref{ex:ez-convergent-sequence}.5).
    \end{enumerate}
\end{exercises}