...
\begin{definition}[开球和闭球]
    ...
    $$
    \B(a,r) \coloneqq \set{x \in X}{d(x,a) < r}
    $$
    是以 $a$ 为中心, $r$ 为半径的开球. 并称集合
    $$
    \bar\B(a,r) \coloneqq \set{x \in X}{d(x,a) \leq r}
    $$
    是以 $a$ 为中心, $r$ 为半径的闭球.
\end{definition}
...
\begin{example}\label{eg:ez-convergent-sequence}
    \begin{enumerate}
        ...
        \item 设复数列 $(z_n)$, 其中
        $$
        z_n \coloneqq \frac{n+2}{n+1} + i \, \frac{2n}{n+2}\ ,
        $$
        则 $\lim z_n = 1 + 2i$.
...
\begin{proof}
    ...
    和
    $$
    \Bigl(\frac{2n}{n+2} - 2\Bigr)^2 = \Bigl(\frac{4}{n+2}\Bigr)^2 < \frac{16}{N^2} < \frac{\varepsilon^2}{4} < \frac{\varepsilon^2}{2}\ ,\quad n \geq N\ ,
    $$
    于是
    \begin{align*}
        |z_n - (1 + 2i)| &= \Bigl|\frac{1}{n+1} - i \, \frac{4}{n+2}\Bigr| \\
        &= \sqrt{\Bigl(\frac{1}{n+1}\Bigr)^2 + \Bigl(-\frac{4}{n+2}\Bigr)^2} < \sqrt{\frac{\varepsilon^2}{2} + \frac{\varepsilon^2}{2}} < \varepsilon\ ,\quad n \geq N\ ,
    \end{align*}
    即当 $n \geq N$ 时 $z_n \in \B\bigl((1 + 2i), \varepsilon\bigr)$.
...
\begin{proof}
    ...
    $$
    n_0 \coloneqq 0\ ,\quad n_k \coloneqq \min\set{m \in \N}{m > n_{k-1}\ \text{且}\ x_m \in \B(a, 1/k)}\ ,\quad k \in \N^\ast\ .
    $$
    由于 $a$ 是 $(x_n)$ 的聚点, 故
    $$
    \set{m \in \N}{m \geq n_{k-1}\ \text{且}\ x_m \in \B(a, 1/k)} \neq \varnothing\ ,\quad k \in \N^\ast\ ,
    $$
...
\begin{exercises}
    设 $X \coloneqq \set{z \in \C}{|z| \leq 3}$ 赋予了自然度量. 描述 $\bar\B_X(0,3)$ 和 $\bar\B_X(2,4)$, 并证明 $\bar\B_X(2,4) \subset \bar\B_X(0,3)$.
\end{exercises}
...
\begin{example}
    \begin{enumerate}
        ...
        \item $\displaystyle \lim_{n \to \infty} \Bigr(\frac{3n}{(2n+1)^2} + i \, \frac{2n^2}{n^2 + 1}\Bigl) = 2i$.
...
\begin{proof}
    ...
    2. 令
    $$
    x_n \coloneqq \frac{3n}{(2n+1)^2} + i \, \frac{2n^2}{n^2 + 1}\ .
    $$
    其中
    $$
    ...
    $$
    而 $\lim (1 + 1/n^2) = 1$, 于是由命题 \ref{prop:addition-scalar-multiplication-in-c} 和命题 \ref{prop:division-in-c} 可知 $\lim \Im x_n = 2$. 最后由 \ref{prop:convergence-of-real-and-imaginary-parts} 可得 $\lim x_n = 2i$.
    ...
    $$
    因此 $\bigl(i^n/(i + 1/n)\bigr)$ 有界, 由命题 \ref{prop:multiplication-in-c} 可知 $(x_n)$ 是无穷小量.
\end{proof}
...
\begin{tikzpicture}
    ...
    \draw[->] (0,0) -- (4,0) node[above]{$-2x$};
    \draw[->] (0,0) -- (-2,0) node[above]{$x$};
    \draw[->] (0,0) -- (-5,0) node[above]{$5x/2$};
\end{tikzpicture}
\end{center}
...
\begin{definition}[开球和闭球]
    设 $a \in E$, $r > 0$. 称
    $$
    \B(a,r) \coloneqq \set{x \in E}{\|x - a\| < r}\quad \text{和}\quad \bar\B(a,r) \coloneqq \set{x \in E}{\|x - a\| \leq r}
    $$
    是以 $a$ 为中心, $r$ 为半径的开球和闭球. 特别的, 称
    $$
    \B \coloneqq \B(0,1) = \set{x \in E}{\|x\| < 1}\quad \text{和}\quad \bar\B \coloneqq \bar\B(0,1) = \set{x \in E}{\|x\| \leq 1}
    $$
    是 $E$ 中的单位开球和单位闭球. 并记
    $$
...
注 \ref{rem:bounded-and-not-bounded-functions}.2 表明, 当 $E$ 不是平凡向量空间时, $\|\!\cdot\!\|_\infty$ 未必是向量空间 $E^X$ 上的范数. 因此我们应该做出如下定义.

\begin{definition}[有界函数空间]
    称
    $$
    B(X,E) \coloneqq \set{u \in E^X}{u\ \text{有界}}\ ,
    $$
    是从 $X$ 到 $E$ 的有界函数空间.
\end{definition}
...
$$
\B_{\R^2} = \set{x \in \R^2}{|x|_\infty \leq 1} = \set{x \in \R^2}{-1 \leq x_1, x_2 \leq 1}\ ,
$$
...
容易验证
$$
\langle x,y \rangle \coloneqq \sum_{j=1}^m x_j \overline y_j\ ,\quad x,y \in \K^m\ (m \in \N^\ast)
$$
