...
\begin{axiom}[环]
    ...
    和乘法运算
    $$
    \cdot \colon R \times R \to R\ ,\quad (a,b) \mapsto a \cdot b\ .
    $$
    若
...
\begin{definition}[交换环]
    称环 $(R, +, \cdot)$ 是交换环, 若 $\cdot$ 具有交换律.
\end{definition}

\begin{definition}[单位环]
    称环 $(R, +, \cdot)$ 是单位环, 若 $\cdot$ 具有单位元.
\end{definition}
...
\begin{axiom}
    ...
    和乘法运算
    $$
    \cdot \colon K \times K \to K\ ,\quad (a,b) \mapsto a \cdot b\ .
    $$
    若
    \begin{enumerate}[label={$(\mathrm F_{\arabic*})$}]
        \item $(K, +, \cdot)$ 是具有单位元的交换环.
        \item 加法单位元 $0$ 不等于乘法单位元 $1$.
        \item 每个 $a \in K \setminus \{0\}$ 都有逆元 $b \in K \setminus \{0\}$ 使得 $a \cdot b = b \cdot a = 1$.
    \end{enumerate}
    则称 $(K, +, \cdot)$ 是域.
\end{axiom}
...
\begin{axiom}[向量空间]
    ...
    $$
    若对于任意 $\lambda, \mu \in K$ 和 $v,w \in V$ 都有
...
\begin{axiom}[代数]
    设 $\mathcal A$ 是 $K$-向量空间, 其上定义了运算
    $$
    \odot \colon \mathcal A \times \mathcal A \to \mathcal A\ ,\quad (a,b) \mapsto a \odot b\ .
    $$
    若
    \begin{enumerate}[label={$(\mathrm A_{\arabic*})$}]
        \item $(\mathcal A, +, \odot)$ 是环.
        \item
    \end{enumerate}
\end{axiom}
