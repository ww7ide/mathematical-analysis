...
\begin{example}\label{eg:plus-on-function-spaces}
...
\begin{theorem}[二项式定理]
    设 $a$ 和 $b$ 是单位环 $R$ 的两个可交换元 (即, $ab = ba$). 则
    $$
...
\begin{definition}[线性映射]
    设 $V$ 和 $W$ 是 $K$-向量空间. 则称函数 $T \colon V \to W$ 是 $K$-线性映射或 $K$-线性的, 若
    $$
...
\begin{example}\label{eg:vector-spaces}
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}[label={$(\mathrm{SS}_{\arabic*})$}]
            \item $U$ 对向量加法封闭, 即 $U + U \subseteq U$.
        ...
        \item 设 $X$ 是集合. 则 $V^X$ 在定义如下运算 (见例 \ref{eg:plus-on-function-spaces}) 后是 $K$-向量空间.
        $$
        (f+g)(x) \coloneqq f(x) + g(x)\ ,\quad (\lambda \cdot f)(x) \coloneqq \lambda \cdot f(x)\ ,\quad x \in X\ ,\quad \lambda \in K\ ,\quad f,g \in V^X\ .
        $$
        特别的, $K^m\ (m \in \N^\ast)$ 在定义如下运算后是 $K$-向量空间.
        $$
        x + y \coloneqq (x_1 + y_1, \ldots, x_m + y_m)\ ,\quad \lambda \cdot x \coloneqq (\lambda \cdot x_1, \ldots, \lambda \cdot x_m)\ ,\quad \lambda \in K\ ,\quad x,y \in K^m\ .
        $$
...
\begin{example}\label{eg:algebras}
    \begin{enumerate}
        \item 设 $B$ 是 $A$ 的非空子集. 若
        \begin{enumerate}[label={$(\mathrm{SA}_{\arabic*})$}]
            \item $B$ 是 $A$ 的子空间.
            \item $B$ 对向量乘法封闭, 即 $B \otimes B \subseteq B$.
        \end{enumerate}
        则称 $B$ 是 $A$ 的子代数.
        \item 设 $X$ 是非空集合. 则 $K^X$ 在定义例 \ref{eg:rings}.2 和例 \ref{eg:vector-spaces}.5 的运算后是具有单位元的交换 $K$-代数.
    \end{enumerate}
\end{example}
