...
\begin{exercise}\label{ex:base-g-expansion-of-natural-numbers}
    ...
    \end{align}
    其中 \(y_k \in \{0, 1, \ldots, g - 1\}\ (0 \leq k \leq \ell)\) 且 \(y_\ell > 0\). 进一步, 表达式 \eqref{exp:base-g-expansion-of-natural-numbers} 是唯一的, 也就是说, 若 \(n = \sum_{j = 0}^m z_j g^j\) 其中 \(z_k \in \{0, 1, \ldots, g - 1\}\ (0 \leq k \leq m)\) 且 \(z_m > 0\), 则 \(\ell = m\) 且 \(y_k = z_k\ (0 \leq k \leq \ell)\).
\end{exercise}
