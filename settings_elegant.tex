...
\DeclareMathOperator{\dom}{dom} % 定义域
\DeclareMathOperator{\im}{im} % 像
\DeclareMathOperator{\ran}{ran} % 值域
\DeclareMathOperator{\id}{id} % 恒同映射
\DeclareMathOperator{\pr}{pr} % 投影
\DeclareMathOperator{\sign}{sign} % 符号
\DeclareMathOperator{\diam}{diam} % 集合的直径
\DeclareMathOperator{\zigzag}{zigzag} % zigzag
\DeclareMathOperator{\closure}{cl} % 闭包
\DeclareMathOperator{\interior}{int} % 内部
...
\ctexset{
    chapter={
        ...
        format=\huge\centering\bfseries\color{structurecolor}
    },
    section={
        format=\Large\centering\bfseries\color{structurecolor}
    },
    subsection={
...
\tcbset{
    ...
    thmstyle/.style={
        ...
        colbacktitle=second,
        fontupper=\slshape,
        % borderline west={2pt}{0pt}{second},
    }
}
% 内部接口
\newtcbtheorem[auto counter,number within=section]{eb@definition}
    {\definitionname}{defstyle}{def}
\newtcbtheorem[use counter from=eb@definition]{eb@axiom}
    {\axiomname}{defstyle}{ax}
\newtcbtheorem[auto counter,number within=section]{eb@proposition}
    {\propositionname}{propstyle}{prop}
\newtcbtheorem[use counter from=eb@proposition]{eb@theorem}
    {\theoremname}{thmstyle}{thm}
\newtcbtheorem[use counter from=eb@proposition]{eb@lemma}
    {\lemmaname}{thmstyle}{lem}
\newtcbtheorem[use counter from=eb@proposition]{eb@corollary}
    {\corollaryname}{thmstyle}{cor}
% 外部接口
\newenvironment{definition}[1][]
    {\begin{eb@definition}{#1}{}}
    {\end{eb@definition}}
\newenvironment{axiom}[1][]
    {\begin{eb@axiom}{#1}{}}
    {\end{eb@axiom}}
\newenvironment{proposition}[1][]
    {\begin{eb@proposition}{#1}{}}
    {\end{eb@proposition}}
...
\newenvironment{corollary}[1][]
    {\begin{eb@corollary}{#1}{}}
    {\end{eb@corollary}}
\newtheoremstyle{colordef}
    {3pt}{3pt} % 上下间距
    {\normalfont}%  正文字体
    {} % 缩进
    {\color{main}\bfseries} % 标题字体：这里上色
    {} % 标题后标点
    {0.5em} % 标题后空格
    {\thmname{#1}\thmnumber{#2}\textnormal{\bfseries#3}} % head spec
\theoremstyle{colordef}
\newtheorem{remark}{注}[section]
\newtheorem{example}[remark]{例}
...
\renewenvironment{proof}[1][\proofname]{\par
    ...
    \item\relax
        {\bfseries
        \textcolor{second}{#1\@addpunct{.}}}\hspace\labelsep\ignorespaces
}{
